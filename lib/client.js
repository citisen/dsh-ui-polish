 ...
 */
const PLUGIN_ID = "@citisen/dsh-ui-polish"
...
function FixSwitch({ label, hint, state, checked, onToggle }) {
  return _react.createElement(
    ...
    _react.createElement(
      ...
      _react.createElement('div', { className: 'dsh-polish-itemLabel' }, label),
      _react.createElement('div', { className: 'dsh-polish-itemHint' }, hint),
    ),
    _react.createElement(
      'div',
      { className: 'dsh-polish-switchRow' },
      _react.createElement('span', { className: 'dsh-polish-state' }, state),
      _react.createElement(
        'button',
        {
          type: 'button',
        ...
        _react.createElement('span', { className: 'dsh-polish-knob' }),
      ),
    ),
  )
}
...
function UiPolishRow({ t, useStore, setField, reset }) {
  ...
  return _react.createElement(
    ...
    _react.createElement(
      ...
      _react.createElement('div', { className: 'dsh-polish-title' }, t('uiPolish.title')),
      _react.createElement('div', { className: 'dsh-polish-desc' }, t('uiPolish.description')),
    ),
    FIXES.map((fix) => {
      const checked = state[fix.id] === true
      return _react.createElement(FixSwitch, {
        key: fix.id,
 ...
 *   by the entry id — `ui-polish`, the row this bundle's patch inserts.
 * - `settingsScope` (0.1.5-rc.x): one scope per registered namespace, addressed by
 *   the name the host half registers — the same `ui-polish` string.
 *
 * Both are read and written here, so the switches behave the same on either line.
 * A composition with neither still gets the fixes on their shipped defaults, and
 * says why the first time a switch is used.
 */
 ...
 * user has to be told that the switch they just flipped is not going to be saved.
...
function reportMissingSettings() {
  ...
  console.error(
    `${PLUGIN_ID}: no settings service this build speaks is present ("${CONFIG_FORMS_SERVICE}" on ` +
      `dsh 0.1.7+, "${SETTINGS_SCOPE_SERVICE}" on the 0.1.5-rc.x line), so the fixes run on their ` +
      'shipped defaults and the switches cannot be saved. Pin dsh to 0.1.5-rc.x (latest/next), or ' +
      'install a newer @citisen/dsh-ui-polish.',
  )
}
...
/**
 * Narrow a raw `ui-polish` section to this plugin's own switches.
 *
 ...
 * hook. Decoding here means `adopt` sees one shape whichever line is running — and
 * an absent setting still means "on", which is what makes an untouched install
 * carry every fix.
 *
 * @param section - the section as stored, if any.
 * @returns the resolved switches, or undefined when there is no section.
 */
function decodePolishSection(section) {
 ...
 * `apply` instead, so a composition with neither still gets the fixes, the row,
 * and a message when a switch is used.
 *
...
function apply(ctx) {
  ...
  // and follow it while it stays: a replacement or an unload puts the fixes back
  // on the defaults. Only the first bind counts, so a composition carrying both
  ...
  const bindSettings = (binding, bound) => {
    ...
    boundSettings = true
    scope = bound
    binding.effect(() => bound.subscribe(adopt), 'dsh-ui-polish: settings adoption')
    adopt()
    return () => {
      boundSettings = false
  ...
  ctx.inject([CONFIG_FORMS_SERVICE], (settingsCtx) =>
    bindSettings(
      settingsCtx,
      decodedForm(settingsCtx[CONFIG_FORMS_SERVICE].get(POLISH_NAMESPACE)),
    ),
  )
  ...
  ctx.slots.inject('settings.general.item', () =>
    ctx.slots.register(
      ...
      UiPolishRow,
		...
		exports.FIXES = FIXES;
		exports.FIX_DEFAULTS = FIX_DEFAULTS;
		exports.HANDLE_SELECTOR = HANDLE_SELECTOR;
		exports.handleFromTarget = handleFromTarget;
		exports.nearestScroller = nearestScroller;
		exports.wheelPixels = wheelPixels;
		exports.canScroll = canScroll;
		exports.wheelPlan = wheelPlan;
		exports.installWheelThrough = installWheelThrough;
		exports.createRowStore = createRowStore;
		exports.UiPolishRow = UiPolishRow;
