...
/**
 * The services this plugin waits for: slots and locale for the settings row, and
 * the settings scope for the switches. Nothing here reads the theme, so the
 * plugin composes without it.
 */
  ...
  ctx.effect(
    () =>
      scope.subscribe(() => {
        ...
        adopt(snapshot.value, snapshot.revision)
      }),
    'dsh-ui-polish: settings adoption',
  )

  const initial = scope.getSnapshot()
  adopt(initial.value ?? FIX_DEFAULTS, initial.revision)
    ...
    ctx.slots.register(
      {
        ...
        inject: (bound) => {
          actions = bound
          const snapshot = scope.getSnapshot()
          adopt(snapshot.value ?? FIX_DEFAULTS, snapshot.revision)
          return {
